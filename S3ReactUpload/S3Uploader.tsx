...
export class S3Uploader extends React.Component<S3UploaderProps, any> {
  ...
  renderFiles() {
    ...
    for (let i = 0; i < files.length; i++) {
      const file = files[i];
      fileItems.push(
        <div key={i} className="row has-borders no-border-top no-border-left no-border-right pad-top-md pad-bottom-md">
          <div className="col-xs-10">
            <p>
              <span>{file.name}</span>
              <span className="pull-right">{this.bytesToSize(file.size)}</span>
            </p>
            <ProgressBar type="info" value={this.getProgress(file)} />
          </div>
